...
describe('DownlineAccessGuard', () => {
  ...
  it('should allow access if user is ADMIN', async () => {
    const context = createMockContext(
      { id: 'admin-id', role: MemberRole.ADMIN },
      { memberId: 'target-id' },
    );
  ...
  it('should allow access if user is SUB_ADMIN', async () => {
    const context = createMockContext(
      { id: 'subadmin-id', role: MemberRole.SUB_ADMIN },
      { memberId: 'target-id' },
    );
    const canActivate = await guard.canActivate(context);
    expect(canActivate).toBe(true);
  ...
  it('should allow access if target member is user self', async () => {
    const context = createMockContext(
      { id: 'user-1', role: MemberRole.MEMBER },
      { memberId: 'user-1' },
    );
    const canActivate = await guard.canActivate(context);
    expect(canActivate).toBe(true);
  ...
  it('should allow access if target member is in user downline', async () => {
    ...
    const context = createMockContext(
      { id: 'user-1', role: MemberRole.MEMBER },
      { memberId: 'child-downline-id' },
    );

    const canActivate = await guard.canActivate(context);
    expect(canActivate).toBe(true);
    expect(hierarchyService.isInDownlineOf).toHaveBeenCalledWith(
      'user-1',
      'child-downline-id',
    );
  });

...
export class RepurchaseCommissionService implements OnModuleInit {
  ...
  async calculateForEntry(
    ...
    if (!repurchaseEntry) {
      throw new NotFoundException(
        `Repurchase entry with ID '${repurchaseEntryId}' not found`,
      );
    }
    ...
    // Active members receive PENDING status; non-ACTIVE / frozen members receive HOLD status (flagged money-in-transit)
  ...
  validateRatesSum(rates: { level: number; percentage: number }[]): void {
    if (rates.length !== 20) {
      throw new BadRequestException(
        `Repurchase commission configuration must contain exactly 20 levels (provided ${rates.length})`,
      );
    }
   ...
   */
  async updateConfig(
    dto: UpdateRepurchaseCommissionConfigDto,
    actorId?: string,
  ) {
    ...
    return this.prisma.$transaction(async (tx: any) => {
      ...
      await tx.repurchaseCommissionConfig.createMany({
        data: dto.rates.map((r) => ({
          ...
          isActive: true,
          description:
            r.description || `Level ${r.level} Repurchase Commission`,
        })),
      });
  ...
  /**
   * Retrieves paginated repurchase commission ledgers with search and filters.
   */
  async findAll(query: any) {
    ...
    if (repurchaseEntryId) where.repurchaseEntryId = repurchaseEntryId;
  ...
  /**
   * Retrieves single repurchase commission ledger by ID.
   */
  async findById(id: string): Promise<any> {
    ...
    if (!ledger) {
      throw new NotFoundException(
        `Repurchase commission ledger with ID '${id}' not found`,
      );
    }

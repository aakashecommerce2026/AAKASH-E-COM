...
export class AdminDistributionController {
  ...
  @ApiResponse({ status: 200, description: 'Paginated pending distribution report and summary totals' })
  ...
  @ApiResponse({ status: 201, description: 'Distribution batch created and processed successfully' })
  @ApiResponse({ status: 400, description: 'No pending ledgers match criteria or validation error' })
  async processBatch(
  ...
  @ApiResponse({ status: 200, description: 'Paginated list of distribution batch history' })
  ...
  @ApiParam({ name: 'batchId', description: 'Distribution Batch UUID or batchNo (e.g. BATCH-20260813-0001)' })

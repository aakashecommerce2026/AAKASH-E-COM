...
describe('HierarchyController', () => {
  ...
  beforeEach(async () => {
    hierarchyService = {
      ...
      getNetworkGrowth: jest.fn().mockResolvedValue([{ period: '2026-08', level: 1, joinCount: 1 }]),
      getBranchCounts: jest.fn().mockResolvedValue([{ branchRootId: 'b-1', totalDownlineInBranch: 5 }]),
      getHierarchySummary: jest.fn().mockResolvedValue({ totalDownline: 10, totalBranches: 2 }),
    };
  ...
  it('searchDownline should delegate to hierarchyService searchDownline method', async () => {
    ...
    expect(hierarchyService.searchDownline).toHaveBeenCalledWith('member-uuid-1', query);
    expect(result).toEqual([mockNode]);
  });
  ...
  it('getNetworkGrowth should delegate to hierarchyService getNetworkGrowth method', async () => {
    ...
    expect(hierarchyService.getNetworkGrowth).toHaveBeenCalledWith('member-uuid-1', query);
    expect(result).toHaveLength(1);
  });

  it('getBranchCounts should delegate to hierarchyService getBranchCounts method', async () => {
    const result = await controller.getBranchCounts('member-uuid-1', { maxLevels: 20 });

    expect(hierarchyService.getBranchCounts).toHaveBeenCalledWith('member-uuid-1', 20);
  ...
  it('getHierarchySummary should delegate to hierarchyService getHierarchySummary method', async () => {
    const result = await controller.getHierarchySummary('member-uuid-1', { maxLevels: 20 });

    expect(hierarchyService.getHierarchySummary).toHaveBeenCalledWith('member-uuid-1', 20);

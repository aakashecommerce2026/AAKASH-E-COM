...
export class HierarchyService {
  private readonly MAX_LEVELS_CAP = 20;

  constructor(private readonly prisma: PrismaService) {}

  /**
   * Fetches full downline referral tree for memberId up to maxLevels (parameterized, capped at 20).
   * Uses high-performance PostgreSQL Recursive CTE via Prisma $queryRaw.
   */
  ...
  /**
   * Walks upline from memberId to the root sponsor up to maxLevels (parameterized, capped at 20).
   * Uses high-performance PostgreSQL Recursive CTE via Prisma $queryRaw.
   * Essential for level-by-level commission calculation & distribution.
   */

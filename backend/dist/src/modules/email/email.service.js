...
let EmailService = EmailService_1 = class EmailService {
    ...
    constructor(configService) {
        ...
        const rawService = this.configService.get('SMTP_SERVICE') || '';
        ...
        if ((host || service) && user && pass) {
            ...
            this.logger.warn('SMTP credentials not fully configured (Set SMTP_USER & SMTP_PASS in .env for live email sending). EmailService running in DEV mode.');
        }
    }
...
`);
    ...
    async sendCommissionEarnedEmail(email, name, amount, commissionType, level, sourceMemberName) {
        ...
        <div style="padding: 25px; background-color: #ffffff;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            <tr style="background-color: #f1f5f9;">
              ...
              <td style="padding: 10px; border: 1px solid #cbd5e1; color: #059669; font-weight: bold; font-size: 18px;">₹${amount.toFixed(2)}</td>
            </tr>
            <tr>
    ...
    async sendPayoutDisbursedEmail(email, name, batchNo, grossAmount, tdsAmount, adminFee, netAmount, paymentRef) {
    ...
    async sendSecurityAlertEmail(email, name, actionDescription, ipAddress) {
    ...
    async sendPasswordResetLinkEmail(email, name, resetLink, token) {
    ...
    async sendAccountDeletionEmail(email, name, memberCode) {
